...
namespace MailGoogle
{
	public class AccountMail:HomePage
	{
		...
		const string CHECK_TERM_XPATH = "//div[@class='xT']/span";
		...
		public void WaitLetter(string termLetter,string textLetter )
		{
			Stopwatch stopwatch = new Stopwatch();
			stopwatch.Start();
			while ((CHeckTermLetter() != termLetter & CheckTextLetter() != textLetter) |
				stopwatch.ElapsedMilliseconds > 600000)
			{
		...
		public void WaitLetterTerm(string termLetter)
		{
			Stopwatch stopwatch = new Stopwatch();
			stopwatch.Start();

			while (CHeckTermLetter() != termLetter |
				stopwatch.ElapsedMilliseconds > 600000)
			{
				_driverGoogle.FindElement(By.XPath(SITE_RELOAD_MAIL_XPATH)).Click();
			}
		...
		{
			var textLetter = FindElementWhithWaiter(CHECK_TERM_XPATH).Text;
		...
		public void Exit()
		{
			...
			allFrameAccount = FindElementsWhithWeiter(SITE_ACCOUNT_EXIT);
